...
public abstract class Controller {
    ...
    * */
    ...
    private void prompt(){
        ...
        while (true){
            try {
                System.out.println("메뉴를 선택하세요(m - 메인메뉴, q - 종료): ");
    ...
    * */
    ...
    /*
    * 모든 컨트롤러는 run()을 호출해서 화면이 전환, 정의된 메서드는 순서는 변경 X
    * - 추상 템플릿 메서드 패턴
    * */
    ...
    * 하위 클래스인 컨트롤러에서 Runable 인터페이스의 구현체를 만들어서 호출
    *
    * @param runnable
    * */
    ...
    public String inputEach(String message, Scanner sc){
        ...
        while (true){
            ...
            if(input != null && !input.isBlank()){
                input = input.trim();
                if(input.equalsIgnoreCase("m")){    //메인 메뉴로 이용
                    Router.change(MainController.class);
                    break;
                }
